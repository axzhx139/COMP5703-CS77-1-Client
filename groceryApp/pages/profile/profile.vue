	...
	export default {
		...
		methods: {
			...
			loadInfo(){
				...
				// 	}
				// })
				this.avatar="http://"+uni.getStorageSync('ip')+"/users/avatar/"+uni.getStorageSync('userId')+'?'+Math.random()
				if(this.avatar==''){
					this.avatar = "../../static/girl.png"
				}
				uni.request({
				...
				uni.request({
					url: "http://"+uni.getStorageSync('ip')+"/ranking/previous/"+uni.getStorageSync('userId')+"/null",
					method: 'get',
				}).then(res=>{
